function [f, z] = findradius(A, epsln)
% Michael Overton and Emre Mengi (Last Update on Spetember 2 2014)
% call: function [f, z] = pspr_2way(A, epsln)
...
if epsln == 0  % pseudospectrum is just the spectrum
   ...
   while ~no_thetaeig & r > rold    % r is increasing in exact arithmetic
      ...
      [r, thetabest] = radiushelper(A, mE, theta, realtol, rold, iter,h);
      ...
      if r > rold
        ...
        thetaold = theta;
      	theta = findtheta(A, mE, epsln, r, thetabest, iter, radtol, ...
          	 smalltol, h);
        if (n >= 30)
   ...
   f = r;
   z = r*(cos(thetabest) + 1i*sin(thetabest));
   
   if isreal(A) & ~isreal(z)
       z = [z; r*(cos(thetabest) - 1i*sin(thetabest))];
   end
